...
3. Build with the existing Developer ID Application identity:
   ...
   ```bash
   AXORC_CODESIGN_IDENTITY='Developer ID Application: ...' scripts/build-release-artifact.sh 0.1.10
   ```

4. Submit `dist/axorc-0.1.10-macos-universal.zip` to `notarytool` using the approved release credentials. Wait for acceptance. Zip archives cannot be stapled; verify the downloaded executable's notarization ticket online after publication.
...
3. Render the formula with the public artifact checksum:
   ...
   ```bash
   scripts/render-homebrew-formula.sh 0.1.10 <sha256> > axorc.rb
   ```
